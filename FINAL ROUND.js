...
export function PestScreen({ navigation }) {
  ...
  const url = "http://192.168.57.201:5000";
...
function MapScreen() {
  ...
  // Dummy data for egg sellers and cocoon sellers
  const eggSellers = [ 
    { id: 1, name: "Egg Seller 1", latitude: 13.013512, longitude: 80.121342 },
    { id: 2, name: "Egg Seller 2", latitude: 13.012728, longitude: 80.115710 },
    
  ];

  const cocoonSellers = [ 
    { id: 1, name: "Cocoon Buyer 1", latitude: 13.011693, longitude: 80.115345 },
    { id: 2, name: "Cocoon Buyer 2", latitude: 13.012153, longitude: 80.114486 },
  ];
  ...
  // Function to handle marker click and show details
  const handleMarkerPress = (sellerType, seller) => {
    Alert.alert(`${sellerType} Details`, `Name: ${seller.name}`);
  };
  ...
  return (
    <View style={styles.containermap}>
      {location ? (
        ...
        >
          {/* Markers for Egg Sellers (Red) */}
          {eggSellers.map((seller) => (
            <Marker
              key={seller.id}
              coordinate={{ latitude: seller.latitude, longitude: seller.longitude }}
              pinColor="yellow" // Red color for egg sellers
              onPress={() => handleMarkerPress("Egg Seller", seller)}
            />
          ))}

          {/* Markers for Cocoon Sellers (Blue) */}
          {cocoonSellers.map((seller) => (
            <Marker
              key={seller.id}
              coordinate={{ latitude: seller.latitude, longitude: seller.longitude }}
              pinColor="blue" // Blue color for cocoon sellers
              onPress={() => handleMarkerPress("Cocoon Buyer", seller)}
            />
          ))}
          ...
          headerRight: () => (
            <View style={{ flexDirection: 'row' }}>
              <Ionicons
                ...
                onPress={() => navigation.navigate('MapScreen')} // Navigate to Map screen
              />
              <Ionicons
                name="document-text"
                size={40}
